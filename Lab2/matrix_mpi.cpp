...
int main( int argc, char* argv[] ) {
  ...
  if ( my_rank == 0 ) {
    for ( int rankItr = 1; rankItr < mpi_size; rankItr++ ) {
      // master sends each partition of a[] to a different slave
      MPI_Send( a + stripe * size * rankItr, stripe * size, MPI_DOUBLE, rankItr, 0, MPI_COMM_WORLD );
      // master also sends b[] to all slaves
      MPI_Send( b, size * size, MPI_DOUBLE, rankItr, 0, MPI_COMM_WORLD );
    }
  }
  else {
  ...
  if (my_rank != 0 ) {
    ...
    for ( int rankItr = 1; rankItr < mpi_size; rankItr++ ) {
      MPI_Recv( c + stripe * size * rankItr, stripe * size, MPI_DOUBLE, rankItr, 0, MPI_COMM_WORLD, &status );
    }
  }
